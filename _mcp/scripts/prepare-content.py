...
for p in (site/'_portfolio').glob('*.md'):
 ...
 projects.append({'id':p.stem,'title':title.group(1).strip('"') if title else p.stem,'url':'https://michaelbailey.org/portfolio/'+p.stem+'/','description':body.strip()})
profile={'name':'Michael Bailey','role':'Senior Advisor, AI at J-PAL, MIT; computational social scientist and economist','email':'mbailey@povertyactionlab.org','website':'https://michaelbailey.org','bio':(site/'_pages/about.md').read_text().split('---',2)[-1],'projects':projects,'public_sources_only':True,'updated':datetime.date.today().isoformat()}
(O/'profile.json').write_text(json.dumps(profile,indent=2))
# SCI country pairs, exactly as published; no rescaling.
rows=json.loads((O/'sci-country-raw.json').read_text());by=collections.defaultdict(list);seen=set()
for r in rows:
