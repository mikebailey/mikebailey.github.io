...
import {load,datasets,sci,migration,socialCapital,crossGenderTies,lab,bookingLinks,base} from './data.js';
...
export function createServer(env){
 const server=new McpServer({name:'michael-bailey',version:'1.2.0'},{instructions});
 ...
 tool('query_cross_gender_ties','Cross-Gender Friending Ratio (CGFR) from the Social Capital Lab: how gender-mixed friendship networks are, by country or US county. Look up places (ISO2 or English country names; five-digit FIPS or "County name, ST") or rank lowest/highest. cutoff is the top-n closest friends the ratio is computed over (default 100). Returns definitions, caveats and citation.',{geography:z.enum(['country','us_county']),places:z.array(short).min(1).max(20).optional(),rank:z.enum(['lowest','highest']).optional(),cutoff:z.union([5,10,25,50,75,100,125,150,175,200].map(n=>z.literal(n))).default(100),limit},a=>crossGenderTies(env,a));
 tool('social_capital_lab','Guide to the Social Capital Lab site (social-connectedness.org): its measures (SCI, Cross-Gender Friending Ratio, Economic Connectedness), what each map tool answers and its URL, downloads, citations, and which MCP tool serves each measure. Use it to route a person to the right page or to cite the lab.',{},()=>lab(env));
...
export default {async fetch(request,env){
 ...
 if(path==='/mcp/health')return wrap(Response.json({status:'ok',service:'michael-bailey-mcp',version:'1.2.0',access:'agents only'}));
 ...
 if(path==='/mcp/lab')return wrap(Response.json(await lab(env)));
